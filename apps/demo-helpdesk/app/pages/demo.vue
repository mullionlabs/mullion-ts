    ...
    <div v-else class="demo-content">
      <div class="demo-header">
        ...
        <p class="demo-description">
          Enter a support ticket to see how Mullion prevents admin notes from leaking into customer
          responses.
        </p>
      </div>

      <UCard variant="outline" class="input-card">
        <template #header>
      ...
      <div v-if="result" class="results-section">
        <div class="results-grid">
          <!-- Admin View -->
          <ResultCard title="Admin View" :confidence="result.adminConfidence" scope="admin">
            <div class="result-content">
              ...
              <div class="internal-notes">
                <UIcon name="i-lucide-lock" class="notes-icon" />
                <p class="result-text">{{ result.internalNotes }}</p>
              </div>

              <h3 class="result-subtitle">Recommended Actions</h3>
              <ul class="actions-list">
                <li v-for="(action, index) in result.recommendedActions" :key="index">
                  {{ action }}
                </li>
              </ul>
            </div>
          ...
          >
            <div class="result-content">
              ...
              <div class="public-response">
                <UIcon name="i-lucide-shield-check" class="response-icon" />
                <p class="result-text">{{ result.publicResponse }}</p>
              </div>
...
const analyzeTicket = async () => {
  ...
  try {
    ...
    error.value = err.data?.message || err.message || 'Failed to analyze ticket';
